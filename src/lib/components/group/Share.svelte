<script lang="ts">
	...
	const isAdmin = groupMembers.find(
		(member) => (member.role === 'admin' || member.role === 'owner') && member.user_id === user?.id
	);
		...
		<div class="mb-4 flex justify-end gap-2">
			{#if isAdmin}
				<form
					action="?/updateRoles"
					method="POST"
					use:enhance={updateMembersAction}
		...
		<div class="space-y-4">
			...
			{#if groupMembers.length === 0}
				<p class="text-muted-foreground">
					No members in your group. Pass around the share link to get started.
				</p>
			{:else}
						...
						<div class="flex items-center justify-between space-x-4 text-muted-foreground">
							...
							{:else}
								...
								>
									...
									<Select.Content>
										{#each roles as role}
											<Select.Item value={role} label={role} class="capitalize">{role}</Select.Item>
										{/each}
									</Select.Content>
								</Select.Root>
							{/if}
